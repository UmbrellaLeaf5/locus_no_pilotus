...
void CHECK_DISTANCES(const PolygonObstacle& poly,
  ...
  for (std::size_t i = 0; i < n - 1; ++i)
    for (std::size_t j = i + 1; j < n; ++j) {
      BOOST_TEST(DistanceBetweenPointsOnPolygon(poly, vertexes[i],
                                                vertexes[j]) == dist[count]);
      ++count;
    }
}

BOOST_AUTO_TEST_SUITE(distance_between_points, *utf::tolerance(1.0E-5))
...
BOOST_AUTO_TEST_CASE(points_on_vertical_line_3) {
  Point point1(3, 10);
  Point point2(3, 14);
  BOOST_TEST(DistanceBetweenPoints(point1, point2) == 4);
}
...
BOOST_AUTO_TEST_CASE(points_on_horizontal_line_3) {
  Point point1(-13, -5);
  Point point2(-13, 8);
  BOOST_TEST(DistanceBetweenPoints(point1, point2) == 13);
}
...
BOOST_AUTO_TEST_CASE(simple_test) {
  ...
  BOOST_TEST(DistanceBetweenPointsOnCircle(circle, point1, point2) ==
             31.415926);
}
